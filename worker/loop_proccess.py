    ...
    try:
        ...
        for i in range(target_len):
            ...
            if i == target_len-1:
                ...
                index = i
                break
            elif (i>0 or tools.check_high_candel(data[i][2], data[i][3], sv.signal.volume*0.25, sv.settings.coin)) and high_tail_1>body_1*2 and data[i][1] < data[i][4] and sv.signal.type_os_signal in ['ham_1by', 'ham_usdc', 'ham_1bx', 'ham_1a', 'ham_1aa', 'ham_5a', 'ham_60c', 'ham_60cc', 'ham_brg'] or 'ham_usdc' in sv.signal.type_os_signal:
                ...
                if vol_can_1>vol_can_2:
                    ...
                    index = i
                    break
            elif (tools.check_high_candel(data[i][1], data[i][4], sv.signal.volume*0.6, sv.settings.coin)) and data[i][1] < data[i][4] and sv.signal.type_os_signal in ['ham_60c', 'ham_60cc', 'ham_usdc', 'ham_brg'] or 'ham_usdc' in sv.signal.type_os_signal:
                type_close = 'upcand'
                cand_close = data[i+1]
                price_close = data[i+1][1]
                index = i
